...
func New() (*Bus, error) {
	...
	b := &Bus{w: w, c: make(chan host.Message), list: map[string][]string{}}
...
// Emit implements host.IR.
func (b *Bus) Emit(remote string, key host.Key) error {
	// http://www.lirc.org/html/lircd.html#lbAH
	_, err := fmt.Fprintf(b.w, "SEND_ONCE %s %s", remote, key)
	return err
}

// Channel implements host.IR.
func (b *Bus) Channel() <-chan host.Message {
	return b.c
}
		...
		if line == "BEGIN" {
			...
			if parts := strings.SplitN(line, " ", 5); len(parts) != 4 {
				...
				if i, err2 := strconv.Atoi(parts[1]); err2 != nil {
					...
					b.c <- host.Message{Key: host.Key(parts[2]), RemoteType: parts[3], Repeat: i != 0}
				}
			}
		}
		if err != nil {
