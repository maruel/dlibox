...
// rampCache is a cache of recent lookup tables.
...
func ensureRampCached(max uint16) *rampTable {
	...
	if len(rampCache) == 256 {
		// We need to trim one. Do it at random for now. That's crappy but faster
		// than implementing a real LRU cache.
		for k := range rampCache {
			delete(rampCache, k)
		}
	}
	r := &rampTable{}
