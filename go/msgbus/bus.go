...
type Bus interface {
	...
	Subscribe(topic_query string, qos QOS) (<-chan Message, error)
	...
	Unsubscribe(topic_query string)

	// Retained retrieves a copy of all matching messages for a retained topic
	// query.
	Retained(topic_query string) ([]Message, error)
}
...
func (r *rebaseSubscriber) Subscribe(topic_query string, qos QOS) (<-chan Message, error) {
	if strings.HasPrefix(topic_query, "//") {
		return r.Bus.Subscribe(topic_query[2:], qos)
	}
	// BUG: Support mergeTopic().
	actual := r.root + topic_query
...
func (r *rebaseSubscriber) Unsubscribe(topic_query string) {
	if strings.HasPrefix(topic_query, "//") {
		r.Bus.Unsubscribe(topic_query[2:])
		return
	}
	// BUG: Support mergeTopic().
	r.Bus.Unsubscribe(r.root + topic_query)
}

func (r *rebaseSubscriber) Retained(topic_query string) ([]Message, error) {
	// BUG: Support mergeTopic().
	msgs, err := r.Bus.Retained(r.root + topic_query)
	if err != nil {
		return msgs, err
	}
	offset := len(r.root)
