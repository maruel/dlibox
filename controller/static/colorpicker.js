...
// Modified by Marc-Antoine Ruel.
// Copyright © 2016 Marc-Antoine Ruel, All rights reserved.
// Remove VML support, removed "no hassle" mode to further reduce code.
(function(window, document, undefined) {
  ...
  function hsv2rgb(hsv) {
    ...
    h = ~~h;
    var R = hsv.v - C;
    var G = R + [X, C, C, X, 0, 0][h];
    var B = R + [0, 0, X, C, C, X][h];
    R += [C, X, 0, 0, X, C][h];
  ...
  function ColorPicker(slideElement, pickerElement, callback) {
    ...
    // Generate uniq IDs for linearGradients so that we don't have the same IDs
    // within one document. Then reference those gradients in the associated
    // rectangles.
