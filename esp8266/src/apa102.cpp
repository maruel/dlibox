...
HomieSetting<long> apa102Hz("apa102Hz", "SPI bus frequency");
HomieSetting<long> apa102Lights("apa102Lights", "Number of lights");
// Do we want serialization? Probably not, MQTT can do it for us.
HomieSetting<long> apa102Intensity("apa102Intensity", "Intensity of the lights (0~255)");
HomieSetting<long> apa102Temperature("apa102Temperature", "Color temperature (K) of lights");

class APA102Node : public HomieNode {
public:
  APA102Node()
      : HomieNode("lights", "apa102") {
    advertise("intensity").settable(
        [](const HomieRange &range, const String &value) {
          apa102Intensity.setDefaultValue(toInt(value, 0, 255));
          return true;
        });
    advertise("temperature").settable(
        [](const HomieRange &range, const String &value) {
          apa102Temperature.setDefaultValue(toInt(value, 2000, 20000));
          return true;
        });
  }

private:
  DISALLOW_COPY_AND_ASSIGN(APA102Node);
};
...
uint32_t Write(const Frame& pixels, uint16_t maxIntensity) {
  ...
  SPI.writeBytes(rawAPA102buffer, rawAPA102bufferLen);
...
void initAPA102() {
  if (!apa102Hz.get()) {
    // The faster means less CPU utilization.
    apa102Hz.setDefaultValue(8000000);
  }
  // Default to non-aggressive mild-warm.
  if (!apa102Intensity.get()) {
    apa102Intensity.setDefaultValue(127);
  }
  if (!apa102Temperature.get()) {
    apa102Temperature.setDefaultValue(5000);
  }
  SPI.setDataMode(SPI_MODE0);
  SPI.setBitOrder(MSBFIRST);
  SPI.begin();
  SPI.setFrequency(apa102Hz.get());
}
